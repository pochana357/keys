...
export default class Log {
  ...
  async fetchPull(
    ...
    const retrieveEvents = <T extends EventType>(
      eventType: T,
      fetchOptions: FetchEventsOptions,
    ) =>
      fetchEventsWithCache(
        eventType,
        this.code,
        ...
        events.map((event) => ({
          ...
          source: this.fights.findUnitRaw(
            event.sourceID,
            event.sourceIsFriendly,
          ),
          target: this.fights.findUnitRaw(
            event.targetID,
    ...
    const castEvents = (
      ...
      if (
        castData.healOnly &&
        event.source &&
        !ClassUtils.isHeal(event.source)
      ) {
        return false;
      }
      if (
        castData.tankOnly &&
        event.source &&
        !ClassUtils.isTank(event.source)
      ) {
        return false;
      }
      if (
        castData.nonTankOnly &&
        event.source &&
        ClassUtils.isTank(event.source)
      ) {
        return false;
      ...
      if (castData.selfCastOnly && event.sourceID !== event.targetID)
        return false;
