<script lang="ts">
  ...
  const numTimeTicks = $derived(
    Math.ceil((events.endTime - events.startTime) / timeTick),
  );
  ...
  let showReceived = $derived(
    options.showReceived ?? AppState.defaultSettings.showReceived,
  );
  let pxPerSec = $derived(
    options.pxPerSec ?? AppState.defaultSettings.pxPerSec,
  );
  const offsetX = (timestamp: number) =>
  ...
  const damageEventsPartitioned = $derived(
    partitionEventsByPlayer(events.damages),
  );
  ...
  const debuffEventsPartitioned = $derived(
    partitionEventsByPlayer(events.debuffs),
  );
</script>
    ...
    {#each events.players as player (player.guid)}
      <div class="sticky left-2 my-1 flex w-max items-center gap-2 font-bold">
        <img
          ...
          alt={player.icon}
        />
        <span style:color={ClassUtils.classColor(player.icon.split('-')[0])}
          >{player.name}</span
        >
      </div>
      ...
      <DamageEventsViewer
        ...
        options={{
          referenceTime,
          damageGroupInterval: options.damageGroupInterval,
          offsetX,
        }}
        {buffDict}
        {exportMode}
