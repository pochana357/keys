<script lang="ts">
  ...
  const damageGroupInterval = $derived(
    options.damageGroupInterval ?? defaultDamageGroupInterval,
  );
  ...
  let mergeGroups = $derived.by(() => {
    ...
    damageTakenEvents.forEach((e, idx) => {
      ...
      if (
        lastTimestamp &&
        e.timestamp - lastTimestamp.timestamp < damageGroupInterval
      ) {
  ...
  <div>
    ...
    <p>
      ...
      {#if SpellSchool.isPhysical(event.ability.type)}
        <span style:color={SpellSchool.getColor(SpellSchool.Physical)}
          >Phys</span
        >
      {/if}
      ...
      {#if SpellSchool.isNature(event.ability.type)}
        <span style:color={SpellSchool.getColor(SpellSchool.Nature)}
          >Nature</span
        >
      {/if}
      ...
      {#if SpellSchool.isArcane(event.ability.type)}
        <span style:color={SpellSchool.getColor(SpellSchool.Arcane)}
          >Arcane</span
        >
      {/if}
      {#if SpellSchool.isShadow(event.ability.type)}
        <span style:color={SpellSchool.getColor(SpellSchool.Shadow)}
          >Shadow</span
        >
      {/if}
    </p>
